...
my $my_req_info = Kynetx::Test::gen_req_info(
    $rid,
    {       #'ip' => '128.187.16.242', # Utah (BYU)
       'referer' =>
'http://www.google.com/search?q=free+mobile+calls&ie=utf-8&oe=utf-8&aq=t&rls=org.mozilla:en-US:official&client=firefox-a',
...
my $init_rule_env = Kynetx::Test::gen_rule_env( { 'city2' => 'Seattle', } );
...
foreach my $case ( @{$testcases} ) {
    ...
    if ( $case->{'type'} eq 'decl' ) {
        ...
        my $good = cmp_deeply( $e, $case->{'expected_val'},
                               "Evaling " . $case->{'krl'} );
