var kobj_fn = 'kobj-static-20090629.js'; var kobj_ts = '163120';(function(){var _jQuery=window.jQuery,_$=window.$;var jQuery=window.jQuery=window.$=function(selector,context){return new jQuery.fn.init(selector,context);};var quickExpr=/^[^<]*(<(.|\s)+>)[^>]*$|^#(\w+)$/,isSimple=/^.[^:#\[\.]*$/,undefined;jQuery.fn=jQuery.prototype={init:function(selector,context){selector=selector||document;if(selector.nodeType){this[0]=selector;this.length=1;return this;}
...
return rm_div;}
function runAnnotate(){var count=0;$K(defaults['results_lister']).each(function(){var contents=annotate(this);if(contents){count++;if($K(this).find('#'+defaults.name+'_anno_list li').is('.'+defaults.name+'_item')){$K(this).find('#'+defaults.name+'_anno_list').append(mk_list_item(defaults.sep)).append(mk_list_item(contents));}else{$K(this).find(defaults['element_to_modify']).prepend(mk_rm_div(contents));}}});KOBJ.logger('annotated_search_results',config['txn_id'],count,'','success',config['rule_name']);cb();}
...
return h;};KOBJ.pick=function(o){if(o){return o[Math.floor(Math.random()*o.length)];}else{return o;}};KOBJ.close_notification=function(s){$K(s).bind("click.kGrowl",function(e){$K(this).unbind('click.kGrowl');$K(s).parents(".kGrowl-notification").trigger('kGrowl.beforeClose').animate({opacity:'hide'},"normal","swing",function(){$K(this).trigger('kGrowl.close').remove();});});};KOBJ['data']=KOBJ['data']||{};KOBJ.proto=(("https:"==document.location.protocol)?"https://":"http://");KOBJ.runtime_host=document.location.host;if(KOBJ.runtime_host=='init.kobj.net'){KOBJ.init_host='init.kobj.net';KOBJ.eval_host='cs.kobj.net';KOBJ.callback_host='log.kobj.net';}else{KOBJ.init_host=KOBJ.runtime_host;KOBJ.eval_host=KOBJ.runtime_host;KOBJ.callback_host=KOBJ.runtime_host;}
KOBJ.init=function(init_obj){$K.each(init_obj,function(k,v){KOBJ[k]=v;});};KOBJ.css=function(css){if($K("style[title=KOBJ_stylesheet]").length==0){$K('<style title="KOBJ_stylesheet"></style>').appendTo('head');}
