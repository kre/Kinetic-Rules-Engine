...
//    console.log("Executing from store_data_sets");
...
{
  var primary_app = KOBJ.get_application(this.app_id);
//  return this.data_sets != null || (this.secondary_run && primary_app.data_sets != null);
  return true;
};
...
KrlApplication.prototype.execute_closure = function(guid, a_closure)
{
    ...
    {
//        console.log("closure execution pending");


        KOBJ.loggers.application.trace("Adding closure to pending list " + this.app_id + " : " + guid);
        this.pending_closures.push(a_closure);
    }
    else
    {
//        console.log("closure executing");
        this.pending_closures.push(a_closure);
        this.execute_pending_closures();
    }
...
KrlApplication.prototype.execute_pending_closures = function()
{
    ...
    if (!this.secondary_run && (!this.is_data_loaded() || !this.are_resources_loaded()))
    {
        return;
    }
    ...
    $KOBJ.each(this.pending_closures, function(guid, the_closure) {
        KOBJ.loggers.application.trace("Executing Closure " + myself.app_is + " - " + guid);
	console.log("Executing Closure " + myself.app_name + " - " + guid);
        try
        {
          (function($){
            (function(jQuery){
    ...
    this.pending_closures = [];
    // remove this from the list of pending apps
//    var app_idx = KOBJ.pending_apps.indexOf(this.app_name);
//    if(app_idx != 1)KOBJ.pending_apps.splice(app_idx,1);
...
KrlApplication.prototype.load_data_sets = function() {
    ...
    if (!this.is_data_loaded() && !this.data_set_load_requested && !this.secondary_run)
    {
        var data_url = KOBJ.proto() + KOBJ.init_host + KOBJ.kns_port + "/js/datasets/" + this.app_id + "/?t=t" +
                        this.page_vars_as_url();
        KOBJ.require(data_url);
    }
  if(this.secondary_run){
    this.store_data_sets();
  }
};
