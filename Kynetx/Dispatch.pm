...
sub calculate_rid_list {
  ...
  $logger->trace("Eventtree key: $eventtree_key");

  if ( is_eventtree_stashed($req_info, $memd, $eventtree_key) ) {
    $logger->trace("Using stashed eventtree");
...
sub get_dispatch_info {
  ...
  my $logger = get_logger();
  $logger->trace( "Expr is a ", sub { Dumper $expr} );
...
sub mk_eventtree_key {
  ...
  my $logger = get_logger();
  my $sig_str = "seed";
  $logger->trace("mk eventree key: ", sub {Dumper($rid_list)});
