package Kynetx::OParser;
# file: Kynetx/OParser.pm
#
...
HTML: /<<.*?>>/s  {$return=Kynetx::OParser::html($item[1]) }
# None of the follow have an appreciable effect on speed
#HTML: <perl_quotelike> {$return=Kynetx::OParser::html($item[1]) }
#HTML: /<<([^>]*+(?:>(?!>)[^>]*+)*+)>>/s {$return=Kynetx::OParser::html($item[1]) }
#HTML:  '<<' <skip:''> /((?!>>).)+/s '>>'  {$return=Kynetx::OParser::html($item[1]) }
#HTML: /<<((?:(?>[^>]+)|>(?!>))*)>>/  {$return=Kynetx::OParser::html($item[1]) }
#HTML: /<<(>[^>]|[^>])+>>/s  {$return=Kynetx::OParser::html($item[1]) }
#HTML: /<<((?!>>).)+>>/s  {$return=Kynetx::OParser::html($item[1]) }
#HTML: /<<((?:[^>]+|>(?!>))*)>>/  {$return=Kynetx::OParser::html($item[1]) }
#HTML: '<<' <skip:undef> / ( [^>] | >(?!>) )* /x '>>' {$return=Kynetx::OParser::html($item[1]) }
JS: /<\|.*?\|>/s  {$return=Kynetx::OParser::javascript($item[1]) }
STRING: /"(\\"|[^"])*"|'[^']*'/ {$return=Kynetx::OParser::string($item[1]) }
VAR:   /[_A-Za-z]\w*/
NUM:   /(-)?\d*\.\d+|\d+/
COMMA: /,/
INCR: /\+=/
...
rule: 'rule' VAR 'is' rule_state '{'
  ...
  {$return = {'name' => $item{VAR},
	      ...
	      'cond' => $item{action}->{'cond'} ||
		        Kynetx::OParser::mk_expr_node('bool','true'),
	      'callbacks' => $item[11][0],
	      'post' => $item[12][0],
...
event_seq: <leftop: event_or ('then'|'before') event_or>
     {$return =
       (defined $item[1][1]) ?
          Kynetx::OParser::build_expr_tree($item[1], 'complex_event')
       :
          $item[1][0]
      }
...
modifier: VAR '=' expr
       ...
       | VAR '=' JS
        {$return=
         {'name' => $item{VAR},
          'value' => Kynetx::OParser::mk_expr_node('JS',$item[3]),
         }
        }
...
equality_expr: <leftop: add_expr predop add_expr>
      {$return =
       (defined $item[1][1]) ?
         Kynetx::OParser::build_expr_tree($item[1],'ineq')
       :
         $item[1][0]
      }
...
add_expr: <leftop: mult_expr add_op mult_expr>
      {$return=
        (defined $item[1][1]) ?
          Kynetx::OParser::build_expr_tree($item[1], 'prim')
        :
          $item[1][0]
       }
...
unary_expr: 'not' unary_expr
	...
	'domain' => $item[1],
        'var' => $item[3],
        'ineq' => '==',
        'expr' => Kynetx::OParser::mk_expr_node('bool','true'),
...
operator_expr: factor operator(s?)
        {$return = Kynetx::OParser::structure_operators($item[1], $item[2]) }
...
factor: NUM
        {$return=Kynetx::OParser::mk_expr_node('num',$item[1]+0)}
      | '-' NUM
        {$return=Kynetx::OParser::mk_expr_node('num',$item[2] * -1)}
      | STRING
        {$return=Kynetx::OParser::mk_expr_node('str',$item[1])}
      | REGEXP
        {$return=Kynetx::OParser::mk_expr_node('regexp',$item[1])}
      | 'true'
        {$return=Kynetx::OParser::mk_expr_node('bool',$item[1])}
      | 'false'
        {$return=Kynetx::OParser::mk_expr_node('bool',$item[1])}
      | VAR '[' expr ']'
        {$return = Kynetx::OParser::mk_expr_node('array_ref',
                         {'var_expr' => $item[1],
                          'index' => $item[3]}
                       )}
      | persistent_var
      ...
      | function_app
      | '[' expr(s? /,/) ']'
        {$return=Kynetx::OParser::mk_expr_node('array',$item[2])}
      | '{' hash_line(s? /,/) '}'
          {$return=Kynetx::OParser::mk_expr_node('hashraw',$item[2])}
      | VAR   # if this isn't after 'true' and 'false' they'll be vars
        {$return=Kynetx::OParser::mk_expr_node('var',$item[1])}
      | '(' expr ')'
        {$return=$item[2]}
      | <error>
...
function_app: namespace VAR '(' expr(s? /,/) ')'
   ...
   | VAR '(' expr(s? /,/) ')' # FIXME: should allow expressions besides VARS
      {$return=
       {'type' => 'app',
        'function_expr' => Kynetx::OParser::mk_expr_node('var',$item{VAR}),
        'args' => $item[3]
       }
      }
...
trail_exp: 'current' var_domain ':' VAR
     {$return=
      {'type' => 'trail_history',
       'offset' => Kynetx::OParser::mk_expr_node('num','0'),
       'domain' => $item[2],
       'name' => $item[4],
      }
     }
