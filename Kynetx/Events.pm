...
my $event_list_threshold = Kynetx::Configure::get_config('EVENT_LIST_THRESHOLD')
  || 100;
...
# Add the data as params
   ...
   #$logger->debug("Processing events for $rids with event ", sub {Dumper $ev});
	...
	foreach my $rid ( split( /;/, $rids ) ) {
		...
		eval {
			process_event_for_rid( $ev, $req_info, $session, $schedule,
				$rid_info );
		};
		if ($@) {
...
sub process_event_for_rid {
		...
		@{ $ruleset->{'rule_lists'}->{$domain}->{$type}->{'rulelist'} } )
	{
		...
		my $next_state =
		  $sm->next_state( $current_state, $ev, $rid, $session,
			$rule->{'name'} );
	...
	if ( $eexpr->{'type'} eq 'prim_event' ) {
		...
		if ( $op eq 'pageview' ) {
			...
			if ( $eexpr->{'pattern'} || $eexpr->{'legacy'} ) {
				...
				$logger->trace( "Pageview filter request: ",
					sub { Dumper( $eexpr->{'filters'} ) } );
				...
				if ( $num == 0 ) {
					...
					$sm =
					  mk_gen_prim( $domain, $op, $eexpr->{'vars'},
						$eexpr->{'filters'} );
					add_filter( $eexpr->{'filters'}, $rule_lists, $domain, $op,
						$rule );
				}
				$logger->trace( "Created: ", sub { Dumper($sm) } );
			}
			else {
				$logger->warn( "Unknown event expression format: ",
					sub { Dumper($eexpr) } );
			}
		}
		elsif ($op eq 'submit'
		...
		{
			$sm =
			  mk_dom_prim( $eexpr->{'element'}, $eexpr->{'pattern'},
				$eexpr->{'vars'}, $op );
			...
			$logger->trace( "Creating Expression event for $domain:$op" );
			$logger->trace( "Eexpr: ", sub { Dumper($eexpr) } );
			$sm =
			  mk_expr_prim( $domain, $op, $eexpr->{'vars'}, $eexpr->{'exp'} );

			add_filter( [ANY], $rule_lists, $domain, $op, $rule );

		}
		else {
			$logger->trace( "Creating primitive event for $domain:$op" );
			$logger->trace( "Eexpr: ", sub { Dumper($eexpr) } );
			$sm =
			  mk_gen_prim( $domain, $op, $eexpr->{'vars'},
				$eexpr->{'filters'} );
			...
			|| $eexpr->{'op'} eq 'notbetween' )
		{
			...
			my $first =
			  compile_event_expr( $eexpr->{'first'}, $rule_lists, $rule );
			my $last =
			  compile_event_expr( $eexpr->{'last'}, $rule_lists, $rule );
			...
			my $sm0 =
			  compile_event_expr( $eexpr->{'args'}->[0], $rule_lists, $rule );
			my $sm1 =
			  compile_event_expr( $eexpr->{'args'}->[1], $rule_lists, $rule );
			if ( $eexpr->{'op'} eq 'and' ) {
				$sm = mk_and( $sm0, $sm1 );
			}
		...
		if ( $eexpr->{'op'} eq 'repeat' ) {
			...
			$logger->trace( "Event Expression: ",
				sub { Dumper( $eexpr->{'args'} ) } );
			if ( ref $eexpr->{'args'} eq "ARRAY" ) {
				...
				foreach my $sm_element ( @{ $eexpr->{'args'} } ) {
					...
					my $tsm =
					  compile_event_expr( $sm_element, $rule_lists, $rule );
