...
sub delete_module_caches {
  ...
  if (defined $msig_list) {
    $logger->info("Flushing module environments for $rid.$version as a caller with module signatures ", sub {Dumper $msig_list});
  ...
  if (defined $mod_re_list) {
    $logger->info("Flushing module environments for $rid.$version as a module with module signatures ", sub {Dumper $mod_re_list});
