...
sub gen_js_expr {
  ...
  case: for ( $expr->{'type'} ) {
		...
		/qualified/ && do {
			...
			my $v =
			  gen_js_datasource( $expr->{'source'}, $expr->{'predicate'},
				$rands );
			return $v;
		};
	}
...
sub eval_js_pre {
	...
	foreach my $decl ( @{$pre} ) {
		$js .=
		  eval_one_decl( $req_info, $rule_env, $rule_name, $session, $decl );
	}
...
sub eval_one_decl {
	...
	my ( $var, $val ) =
	  eval_js_decl( $req_info, $rule_env, $rule_name, $session, $decl );
...
sub gen_js_callbacks {
	...
	return join( "",
		map { gen_js_callback( $_, $txn_id, $type, $rule_name, $rid ) }
		  @{$callbacks} );
...
sub eval_js_expr {
	...
	if ( $expr->{'type'} eq 'str' ) {
		...
		$logger->debug( $rule_name . ':' . $expr->{'val'},
			" -> ", $v, ' Type -> ', infer_type($v) );
		return {
			'type' => infer_type($v),
			'val'  => $v
		...
		return {
			'type' => 'array',
			'val'  => eval_js_rands(
				$expr->{'val'}, $rule_env, $rule_name, $req_info, $session
			)
		};
	}
		...
		return Kynetx::Operators::eval_operator( $expr, $rule_env, $rule_name,
			$req_info, $session );
	}
	elsif ( $expr->{'type'} eq 'condexpr' ) {
		return eval_predicates( $req_info, $rule_env, $session, $expr->{'test'},
			$rule_name )
		  ? eval_js_expr( $expr->{'then'}, $rule_env, $rule_name, $req_info,
			$session )
		  : eval_js_expr( $expr->{'else'}, $rule_env, $rule_name, $req_info,
			$session );
	}
	elsif ( $expr->{'type'} eq 'qualified' ) {
		my $den =
		  eval_js_rands( $expr->{'args'}, $rule_env, $rule_name, $req_info,
			$session );
		...
		my $v =
		  eval_datasource( $req_info, $rule_env, $rule_name, $expr->{'source'},
			$expr->{'predicate'}, $den );

		$logger->debug( "[JS Expr] ", $expr->{'source'}, ":",
			$expr->{'predicate'}, " -> ", $v );
	...
	{
		my $v =
		  eval_persistent( $req_info, $rule_env, $rule_name, $session, $expr );
		return {
			'type' => infer_type($v),
			'val'  => $v
		};
...
sub eval_js_prim {
	...
	my $vals =
	  eval_js_rands( $prim->{'args'}, $rule_env, $rule_name, $req_info,
		$session );
...
sub eval_js_rands {
	...
	my @rands =
	  map { eval_js_expr( $_, $rule_env, $rule_name, $req_info, $session ) }
	  @{$rands};
...
sub eval_js_hash {
	...
	foreach my $hl ( @{$hash_lines} ) {
		$hash->{ $hl->{'lhs'} } =
		  eval_js_expr( $hl->{'rhs'}, $rule_env, $rule_name, $req_info,
			$session );
	}
...
sub eval_js_decl {
	...
	if ( $decl->{'type'} eq 'expr' ) {
		...
		my $r =
		  eval_js_expr( $decl->{'rhs'}, $rule_env, $rule_name, $req_info,
			$session );
...
sub eval_datasource {
	...
	if ( $source eq 'weather' ) {
		...
		$val =
		  Kynetx::Predicates::Useragent::get_useragent( $req_info, $function );
	}
	elsif ( $source eq 'page' ) {
		$val =
		  Kynetx::Predicates::Page::get_pageinfo( $req_info, $function, $args );
	}
...
sub eval_persistent {
	...
	if ( $expr->{'domain'} eq 'ent' ) {
		if ( defined $expr->{'offset'} ) {
			...
			my $idx = den_to_exp(
				eval_js_expr(
					$expr->{'offset'}, $rule_env, $rule_name,
					$req_info,         $session
				)
			);

			$v =
			  session_history( $req_info->{'rid'}, $session, $expr->{'name'},
				$idx );
...
sub infer_type {
	...
	if ( $v =~ m/^(\d*\.\d+|[1-9]\d+|\d)$/ )
	{    # crude type inference for primitives
		$t = 'num';
	}
...
sub var_free_in_expr {
		...
		|| $expr->{'type'} eq 'qualified' )
	{
		...
		return at_least_one(
			[ $expr->{'test'}, $expr->{'then'}, $expr->{'else'} ], $var );
	}
	else {
		return 1;
	}
